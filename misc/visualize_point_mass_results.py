...
def performance_plot(ax, base_log_dir="logs"):
    ...
    for method, color in zip(["goal_gan", "alp_gmm", "acl", "plr", "vds"], ["C6", "C7", "C8", "C9", "C10"]):
        exp = PointMass2DExperiment(base_log_dir, method, "ppo", {}, seed=0)
        log_dir = os.path.dirname(exp.get_log_dir())
        lines.append(add_plot(log_dir, ax, color))
...
def full_performance_plot(ax=None, path=None, base_log_dir="logs"):
    ...
    if show:
        f.legend(lines,
                 [r"\sprl", "Random", "Default", r"\currot", r"\currot", r"\goalgan", r"\alpgmm", r"\acl", r"\plr", r"\vds"],
...
def precision_comparison(axs=None, path=None, base_log_dir="logs"):
    ...
    if show:
        f.legend(lines,
                 [r"\sprl", "Random", r"\currot", r"\currot", r"\goalgan", r"\alpgmm", r"\acl", r"\plr", r"\vds"],
...
def combined_plot(path=None, base_log_dir="logs"):
    ...
    f.legend(lines,
             [r"\sprl", "Random", "Default", r"\currot \tiny{(Prev)}", r"S\currot \tiny{(Ours)}", r"\goalgan", r"\alpgmm", r"\acl", r"\plr", r"\vds"],
