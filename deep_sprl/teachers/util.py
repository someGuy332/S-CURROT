...
class RewardEstimatorGP:
    ...
    def update_model(self, contexts, rewards):
        ...
        self.x = torch.from_numpy(contexts).float().to(device)  # .type(torch.float64)
        self.y = torch.from_numpy(rewards).float().to(device)  # .type(torch.float64)

        if self.gp is None:
            gpytorch.settings.cholesky_jitter(1e-3)
            likelihood = gpytorch.likelihoods.GaussianLikelihood().to(device)
            self.gp = ExactGPModel(self.x, self.y, likelihood).to(device)
        else:
            gpytorch.settings.cholesky_jitter(1e-3)
            old_gp = self.gp
            likelihood = gpytorch.likelihoods.GaussianLikelihood().to(device)
    ...
    def predict_individual(self, samples, with_gradient=False):
        device = next(self.gp.parameters()).device
        samples_torch = torch.from_numpy(samples).float().to(device)
        if with_gradient:
        ...
        if with_gradient:
            ...
            return perf.detach().cpu().numpy().astype(samples.dtype), grad.detach().cpu().numpy().astype(samples.dtype)
        else:
            return perf.detach().cpu().numpy().astype(samples.dtype)
    ...
    def __call__(self, samples, with_gradient=False):
        ...
        if with_gradient:
            ...
            return mean.detach().cpu().numpy().astype(samples.dtype), grad.detach().cpu().numpy().astype(samples.dtype)
        else:
            return mean.detach().cpu().numpy().astype(samples.dtype)
