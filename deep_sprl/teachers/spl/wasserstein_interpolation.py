...
class WassersteinInterpolation:
    ...
    def wasserstein_distance(self, initial_samples, target_samples, for_backprop=False):
        ...
        x = torch.from_numpy(initial_samples).float().to(device).requires_grad_(True)
        alpha = torch.ones(x.shape[0], dtype=x.dtype, device=device) / x.shape[0]
        y = torch.from_numpy(target_samples).float().to(device)
        beta = torch.ones(y.shape[0], dtype=y.dtype, device=device) / y.shape[0]

        if for_backprop:
            return self.sl(alpha, x, beta, y), x
        else:
            return self.sl(alpha, x, beta, y).cpu().detach().numpy()
    ...
    def compute_transport_plan(self, initial_samples, target_samples):
        ...
        return -(g * x.shape[0]).detach().cpu().numpy()
    ...
    def update_distribution_perturbed(self, model, perturbed_samples_list, success_samples):
        # init_samples = self.current_samples.copy()
        # low_perfs = model.predict_individual(init_samples) < self.perf_lb
        # value_plans = []
        # for success_samples in perturbed_samples_list:
        #     if np.any(low_perfs):
        #         value_plans.append(self.compute_transport_plan(init_samples, success_samples))
        #         init_samples[low_perfs, :] = init_samples[low_perfs, :] + value_plan[low_perfs, :]
        # value_plan = np.mean(value_plans, axis=0) if len(value_plans) > 0 else np.zeros_like(init_samples)
        # init_samples[low_perfs, :] = init_samples[low_perfs, :] + value_plan[low_perfs, :]
        # target_samples = self.target_sampler(self.n_samples)

        init_samples = self.current_samples.copy()
        temp_samples = self.current_samples.copy()
        low_perfs = model.predict_individual(init_samples) < self.perf_lb
        init_sample_list = []
        for iter_samples in perturbed_samples_list:
            if np.any(low_perfs):
                value_plan = self.compute_transport_plan(init_samples, iter_samples)
                init_samples[low_perfs, :] = temp_samples[low_perfs, :] + value_plan[low_perfs, :]
            init_sample_list.append(init_samples.copy())
        target_samples = self.target_sampler(self.n_samples)

        gradient_list = []
        import time
        t1 = time.time()
        for init_samples in init_sample_list:
            ...
            if np.sum(opt_mask) == 0:
                ...
                x = np.clip(x, 0., 1.)
                gradient = x[:, None] * self.target_plan
                gradient_list.append(gradient)
        

        new_samples = success_samples + np.mean(gradient_list, axis=0)
        
        t2 = time.time()
        print("Optimization took: %.3e s" % (t2 - t1))
        self.wass_dist = self.wasserstein_distance(new_samples, target_samples)

        print("Optimized performance: %.3e" % model(new_samples))
        print("Optimized target distance: %.3e" % self.wass_dist)
