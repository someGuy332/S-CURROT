    ...
    def update_distribution(self, contexts, returns):
        ...
        if self.threshold_reached or self.model(self.teacher.current_samples) >= self.teacher.perf_lb:
            ...
            if self.smooth:
                ...
                smoothed_perf = self.model(smoothed_samples)
                ...
                if smoothed_perf < self.teacher.perf_lb:
                    ...
                    alpha = 0.5  # Projection step size
                    smoothed_samples = alpha * smoothed_samples + (1 - alpha) * self.teacher.current_samples
                    ...
                    #     smoothed_samples = alpha * smoothed_samples + (1 - alpha) * self.teacher.current_samples
                self.teacher.current_samples = smoothed_samples  # Update with smoothed samples
        ...
        if self.smooth:
            ...
            if self.iteration > 400:
                self.sigma_t = self.sigma_t * 0.99
                pass
            else:
                if self.local_minima:
                    ...
                    self.sigma_t = self.sigma_t * 0.99 # Decay variance
